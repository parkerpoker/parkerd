...
describe("MeshRuntime", () => {
  ...
  it("plays a daemon-to-daemon hand, publishes to the public indexer, and replays after restart", async () => {
    ...
    const host = new MeshRuntime("host", buildConfig(cleanupDir, indexerUrl, hostPort!), new CliLogger(true));
    const witness = new MeshRuntime("witness", buildConfig(cleanupDir, indexerUrl, witnessPort!), new CliLogger(true));
    const alpha = new MeshRuntime("alpha", buildConfig(cleanupDir, indexerUrl, alphaPort!), new CliLogger(true));
    const beta = new MeshRuntime("beta", buildConfig(cleanupDir, indexerUrl, betaPort!), new CliLogger(true));
    ...
    try {
      ...
      await waitFor(async () => {
        const response = await fetch(`${indexerUrl}/api/public/tables`);
      ...
      alpha.close();
      const replayed = new MeshRuntime("alpha", buildConfig(cleanupDir, indexerUrl, replayPort!), new CliLogger(true));
      try {
...
function buildConfig(root: string, indexerUrl: string, peerPort: number): CliRuntimeConfig {
  return {
