...
export function resolveCliRuntimeConfig(flags: CliFlagMap): CliRuntimeConfig {
  ...
  const indexerUrl =
    typeof flags["indexer-url"] === "string"
      ? flags["indexer-url"]
      : process.env.PARKER_INDEXER_URL ?? process.env.VITE_INDEXER_URL;
  const nigiriDatadir =
  ...
  const profileDir = resolve(
    typeof flags["profile-dir"] === "string"
      ? flags["profile-dir"]
      : process.env.PARKER_PROFILE_DIR ?? "apps/daemon/data/profiles",
  );
  const daemonDir = resolve(
    typeof flags["daemon-dir"] === "string"
      ? flags["daemon-dir"]
      : process.env.PARKER_DAEMON_DIR ?? "apps/daemon/data/daemons",
  );
  const runDir = resolve(
    typeof flags["run-dir"] === "string"
      ? flags["run-dir"]
      : process.env.PARKER_RUN_DIR ?? "apps/daemon/data/runs",
  );
