...
import (
	...
	"github.com/danieldresner/arkade_fun/internal/parker/game"
)
...
func NewProxyDaemon(profileName string, config RuntimeConfig, mode string) *ProxyDaemon {
	return &ProxyDaemon{
		config:      config,
		mode:        mode,
		...
		cachedState: map[string]any{},
		runtime:     newNativeRuntime(profileName, config, mode),
	}
}

func (d *ProxyDaemon) Start() error {
	if existing, err := ReadProfileDaemonMetadata(d.paths); err == nil && existing != nil && existing.PID != os.Getpid() && !IsPidAlive(existing.PID) {
		if err := CleanupDaemonArtifacts(d.paths); err != nil {
			return err
		}
	}

	if err := os.MkdirAll(d.paths.StateDir, 0o755); err != nil {
		return err
	}
	...
	d.metadata = metadata
	if err := WriteProfileDaemonMetadata(d.paths, *metadata); err != nil {
		_ = listener.Close()
		return err
	}
...
func (d *ProxyDaemon) Stop() error {
	...
	d.stopOnce.Do(func() {
		...
		stopErr = errors.Join(stopErr, CleanupDaemonArtifacts(d.paths))
		close(d.stoppedCh)
	})
	return stopErr
}
...
func (d *ProxyDaemon) runPoller() {
	ticker := time.NewTicker(time.Duration(nativePollIntervalMS) * time.Millisecond)
	defer ticker.Stop()
...
func (d *ProxyDaemon) writeRunningMetadata() error {
	...
	return WriteProfileDaemonMetadata(d.paths, *d.metadata)
}
