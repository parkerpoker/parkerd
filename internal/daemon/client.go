...
func NewClient(profileName string, config cfg.RuntimeConfig) *Client {
	return &Client{
		Config:      config,
...
func (c *Client) EnsureRunning(mode string) error {
	...
	paths := cfg.BuildProfileDaemonPaths(c.Config.DaemonDir, c.ProfileName)
	if metadata, err := cfg.ReadProfileDaemonMetadata(paths); err == nil && metadata != nil && cfg.IsPidAlive(metadata.PID) {
...
func (c *Client) Inspect(autoStart bool) (map[string]any, error) {
	paths := cfg.BuildProfileDaemonPaths(c.Config.DaemonDir, c.ProfileName)
	metadata, err := cfg.ReadProfileDaemonMetadata(paths)
	if err != nil {
		return nil, err
	}
...
func (c *Client) startDaemonProcess(mode string) error {
	...
	if metadata != nil && !cfg.IsPidAlive(metadata.PID) {
		if err := cfg.CleanupDaemonArtifacts(paths); err != nil {
			return err
		}
	}
	...
	launcherPath, err := cfg.ResolveLauncherPath("parker-daemon")
	if err != nil {
		return err
	}
...
func startFailureDetails(paths cfg.ProfileDaemonPaths) string {
	details := make([]string, 0, 2)
	if metadata, err := cfg.ReadProfileDaemonMetadata(paths); err == nil && metadata != nil {
...
func startDirectory() string {
	if root, err := cfg.FindRepoRoot(); err == nil {
		return root
	}
