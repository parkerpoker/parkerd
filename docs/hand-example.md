...
This document gives a concrete, end-to-end view of how money moves through a Parker heads-up table today.
...
| `emergency-exit` | No new cooperative batch | Uses the current source refs plus a local exit execution proof |
...
- if canonical money refs do not change, Parker can still advance custody with approvals and replay validation, but without a new Ark batch
...
- the cooperative leaf requires every currently required player signer plus the operator signer
...
Parker may still create non-money custody updates later that bind those fields, but the card protocol itself does not respend the pot or stack refs.
...
The custody state may still bind the new transcript root and public state hash, but no Ark batch is required just because community cards were revealed.
...
These are still accepted custody transitions, but they are zero-exposure transitions:
...
If Alice disappears and the action deadline passes:
...
- cooperative branches require the currently required player set plus the operator signer
...
- the winner's value is now in the winner's stack claim
...
The cleanest way to think about Parker money flow today is:
